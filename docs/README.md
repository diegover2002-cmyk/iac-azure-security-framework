# Documentation Index

This directory is the canonical documentation set for the repository.

## Start Here

- [Introduction](introduction.md)
- [Repository Structure](repository-structure.md)
- [Control Matrix Guide](control-matrix-guide.md)
- [Cross-Cutting Domains](cross-cutting-domains.md)
- [Service Catalog Maturity Roadmap](service-catalog-maturity-roadmap.md)
- [Catalog Normalization Notes](catalog-normalization-notes.md)
- [Modules Repo Integration Plan](modules-repo-integration.md)
- [Adding New Services](adding-new-services.md)
- [Automation and Validation](automation-and-validation.md)
- [Checkov Coverage Audit](checkov-coverage-audit.md)
- [Enterprise MCSB Audit Operating Model](enterprise-mcsb-audit-operating-model.md)
- [Manual CI/CD Process](manual-ci-cd-process.md)
- [Testing Guide](testing-guide.md)
- [References](references.md)
- [AI Agent Guide](ai-agent-guide.md)
- [Documentation Strategy](documentation-strategy.md)

## Documentation Rules

- `docs/` is the source of truth for contributor and process documentation.
- `wiki/` contains the GitHub Wiki pages derived from this structure.
- Root-level documentation is limited to repository entry points and contributor rules.
